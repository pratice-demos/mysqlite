...
*/
const uint32_t LEAF_NODE_KEY_SIZE = ID_SIZE;
...
uint32_t* internal_node_child(void* node, uint32_t child_num) {
  ...
  if (child_num > num_keys) {
    ...
    if (*child == INVALID_PAGE_NUM) {
      printf("Tried to access child %d of node, which was invalid page.\n", child_num);
      exit(EXIT_FAILURE);
    }
    return child;
  }
...
// (void*) is required, otherwise pointer arithmetic will be done in 4 bytes
...
void ident(uint32_t level) {
  for (u_int32_t i = 0; i < level; i++) {
    printf("  ");
  }
}

void print_tree(Pager* pager, uint32_t page_num, uint32_t ident_level) {
  ...
  case NODE_INTERNAL:
    ...
    if (num_keys > 0) {
      for (uint32_t i = 0; i < num_keys; i++) {
        ...
        print_tree(pager, child, ident_level + 1);
...
Cursor* leaf_node_find(Table* table, uint32_t page_num, uint32_t key) {
  ...
  // Binary search, returns the position where new node should be inserted
...
uint32_t internal_node_find_child(void* node, uint32_t key) {
  uint32_t num_keys = *internal_node_num_keys(node);
  /**
   * Binary search to find index of child which should contain key
  */
  uint32_t min_index = 0;
...
void internal_node_split_and_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
  ...
  /**
   * now the node's meaning:
   * old_node (parent_page) -> child (child_page)
   * new_node (new_page) is to be hanged
  */
  ...
  */
  ...
  /**
   * now the node's meaning:
   * parent -> old_node (left child)
   *        -> new_node (right child) = cur
  */


  /**
   * using cur to copy right parts of old_node to new_node
   * then the old_node becomes the left child
  */
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
  ...
  *(leaf_node_num_cells(new_node)) = LEAF_NODE_RIGHT_SPLIT_COUNT;
  /**
   * Update parent node or create a new parent node
  */
...
ExecuteResult execute_insert(Statement* statement, Table* table) {
  // first find the leaf node where the row should be inserted
  Row* row_to_insert = &(statement->row_to_insert);
  uint32_t key_to_insert = row_to_insert->id;
  Cursor* cursor = table_find(table, key_to_insert);

  // insert the row into the leaf node
